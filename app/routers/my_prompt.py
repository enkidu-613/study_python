...
class MyTaskExtractionResult(BaseModel):
    type:str = Literal["question","bug","complaint","feature","praise"]
...
def build_task_extractor():
    ...
    llm = ChatDeepSeek(
        ...
        api_base=os.getenv("MODELSCOPE_API_BASE", "https://api.modelscope.cn"),
        api_key=api_key,
        temperature=0.7, # 决定了生成文本的随机性，值越高，生成的文本越多样化
        streaming=False,
    )
...
async def extract_task_from_text(text: str) -> MyTaskExtractionResult:
    ...
    # ainvoke 是异步启动 langchain 管道对象的方法，可以在启动的时候传入参数，
    # 比如 {"text": text}，表示要处理的文本内容。返回值是一个字典，包含了结构化输出的结果。
