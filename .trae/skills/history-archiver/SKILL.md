...
负责将学习对话按正确日期归档到 `.trae/memory/conversations/YYYY-MM-DD.md`，
并同步更新 `.trae/memory/learning_history_index.json`。
...
2. **确定文件名**：`.trae/memory/conversations/{archive_date}.md`
...
7. **跨框架同步**：如需要，按 `chapter-review-quizzer` 的跨框架规范同步到 `.reasonix/`
...
- [ ] index 中的 `file_path` 指向正确的 `.trae/memory/conversations/YYYY-MM-DD.md`
