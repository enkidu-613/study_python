...
对话记录不再存入单一 `conversations.md`，而是按日期拆分到独立文件：

```
 .trae/memory/
 ├── conversations.md              ← 索引文件（目录/速查）
...
**文件命名格式**: `yyyy-mm-dd.md`（如 `2026-05-28.md`）

**确定对话所属日期**: 以对话实际发生的日期为准（即历史记录中的对话日期），而非当前系统日期。

**存在检查与追加**:
- 若对应日期的 `yyyy-mm-dd.md` 文件已存在 → **追加**对话内容到该文件
- 若对应日期的文件不存在 → **新建**该日期的文件并写入对话内容

**跨日期对话处理**: 若一批新对话跨越了多个日期，则按日期拆分，分别检查并写入对应日期的文件。

**示例**:
```
今天是 2026-05-30，有一条昨天（2026-05-29）的对话需要归档：
→ 检查 2026-05-29.md 是否存在
  - 存在 → 追加写入
  - 不存在 → 新建 2026-05-29.md 并写入

一批对话分别发生在 2026-05-28、2026-05-29、2026-05-30：
→ 依次检查 2026-05-28.md、2026-05-29.md、2026-05-30.md
  - 存在的追加，不存在的新建
```
...
2. **追加到当日对话文件**
   ...
   - 按归档判断逻辑：存在则追加，不存在则新建（标题 `# YYYY-MM-DD 对话历史`）
...
3. **更新 conversations.md 索引**
   - 在「文件索引」表中更新当日文件的对话范围和主题
   - 在「对话列表」表中新增一行
...
- **按日期拆分**: 每天一个文件，方便按时间线回溯
- **简洁**: 只保留精华，不需要完整对话记录
- **结构化**: 使用统一的格式，方便后续检索
- **可追溯**: 包含时间戳和上下文，知道当时学到了什么
- **可行动**: 记录下一步建议或待办事项
- **索引同步**: 每次新增对话，必须同步更新 `conversations.md` 索引表
