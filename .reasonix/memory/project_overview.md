...
### 🔥 当前进行中
- [ ] prompt-advanced（高级提示词工程）
- [ ] pytest进阶与测试完善
- [ ] 错题复习与薄弱点巩固

### 📋 待学习（后续阶段）
- [ ] Prompt Engineering进阶（结构化输出、采样参数、Prompt Injection边界）
...
- ✅ pytest单元测试（FastAPI TestClient、依赖覆盖、fixture、conftest.py、测试数据库、SSE流式测试、WebSocket测试、Arrange/Act/Assert模式、假阳性空测试诊断）
- ✅ 本地Embedding部署与MPS硬件加速
- ✅ 配置集中化（.env环境变量管理）
- ✅ 章节测试与补考系统（试卷/、答题/、错题本）

### 已知薄弱点
- Alembic: 迁移≠备份，空库upgrade需要create_table迁移记录
- JWT: algorithms=["HS256"]是算法白名单防算法混淆攻击，bcrypt哈希结构($2b$+cost+salt+hash)
- Async: create_task不await会隐藏异常，gather(return_exceptions=True)返回异常值，Future是一次性结果占位符，Task是主动运行协程的Future
- RAG: enumerate(start=1)生成引用编号，yield流式vs return一次性返回
- WebSocket: Python用!=而非!==，真正房间分组用dict[str, list[WebSocket]]

### 待深入学习
- ⬜ 高级提示词工程（结构化输出、采样参数、Prompt Injection防御）
- ⬜ LangChain Memory对话记忆注入
- ⬜ LangChain Agents工具调用、LangGraph
- ⬜ RAG Chunking策略优化
- ⬜ RAG评估指标(RAGAS)
- ⬜ AI Agents基础（手动实现优先）
- ⬜ LLM可观测性与评估
...
## 教学偏好
- ADHD友好但精确：类比只在有用时使用，然后讲精确机制
- 短段落、表格、可运行模板、"复制这个模式"式指导效果好
- 不优化记忆，优化：概念、使用场景、原因、模板位置
- 命令精确拼写不需要死记，概念和安全工作流更重要
