...
| 文件 | 关键 async 代码 |
|------|---------------|
| `routers/ai_router.py` | `async def generate_stream` + `async def chat` |
| `routers/rag_router.py` | `async def generate_rag_stream` + `async def rag_chat` |
| `routers/langchain_rag_router.py` | `async def _generate_stream` + `async def langchain_chat` |
